...
def test_build_help_runtime_model_and_registration_are_removed() -> None:
    assert not (ROOT / "src/marwie_bot/features/build_help").exists()
    models = (ROOT / "src/marwie_bot/db/models.py").read_text()
    assert "ForumSolution" not in models
    assert "forum_solutions" not in models
    bot = (ROOT / "src/marwie_bot/bot.py").read_text()
    analytics = (ROOT / "src/marwie_bot/features/analytics/cog.py").read_text()
    assert "build_help" not in bot.casefold()
    assert "build_help" not in analytics.casefold()
    assert "ForumSolution" not in analytics
